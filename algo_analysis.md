...
```sh
	...
	for (int i = 5; i <= 50; i += 5){
			...
			su_given(A, Q, R, i, i);
			temps_final = clock () ;
			temps_cpu1 = ((double)(temps_final - temps_initial))
			/CLOCKS_PER_SEC; su_free_mat(i, Q); su_free_mat(i, R); su_free_mat(i, A);
			...
			temps_final = clock () ;
			temps_cpu2 = ((double)(temps_final - temps_initial))
			/CLOCKS_PER_SEC; su_free_mat(i, A);
...
```sh
    ...
    for (int i = 2; i <= 50; i += 2) {
      ...
      // time for generating matices algorithm
      temps_initial = clock();
