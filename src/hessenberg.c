...
/**
 * Iterative algorithm that does not work because j does not decrement as seen
 *in the proofs
 **/
...
double **su_hessenberg_rec(double **A, int size, int n, double threshold) {
  ...
  return su_hessenberg_rec(su_mul_mat_3(size, G, A, Gt), size, n - 1,
                           threshold);
}
